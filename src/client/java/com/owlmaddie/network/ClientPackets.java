...
public class ClientPackets {
    ...
    public static Map<UUID, PlayerData> readPlayerDataMap(PacketByteBuf buffer) {
        int size = buffer.readInt(); // Read the size of the map
        Map<UUID, PlayerData> map = new HashMap<>();
        for (int i = 0; i < size; i++) {
            UUID key = buffer.readUuid(); // Read the key (UUID)
    ...
    public static void register() {
        ...
        ClientPlayNetworking.registerGlobalReceiver(ServerPackets.PACKET_S2C_ENTITY_MESSAGE,
                (client, handler, buffer, responseSender) -> {
                    ...
                    Map<UUID, PlayerData> players = readPlayerDataMap(buffer);
        ...
        ClientPlayNetworking.registerGlobalReceiver(ServerPackets.PACKET_S2C_PLAYER_MESSAGE,
                (client, handler, buffer, responseSender) -> {
                    ...
                    client.execute(() -> { // Make sure to run on the client thread
                        ...
                        // Optional<Entity> entityToSendChatTo =
                        // ClientEntityFinder.getClosestEntityToPlayerWithChatBubbleOpen();
